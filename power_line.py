...
plt.title("Proposed Graph:Social Network\nNode size = Degree | M = Mixing pattern | P = Content polarity | δ = Partisan status | C = Clustering | EI = Node EI Index")
plt.axis('off')
...
print("="*120)
print(df[['User ID', 'Political Leaning', 'Node EI Index', 
          'Clustering Coefficient', 'Mixing Pattern']].sort_values('Node EI Index', ascending=False))
print("\n" + "="*120)
...
# Define desired order and corresponding colors
desired_order = ['Government', 'Opposition', 'Neutral']
color_map = {"Government": "red", "Opposition": "blue", "Neutral": "green"}
...
high_active_counts = high_active_counts.reindex(desired_order, fill_value=0)
colors = [color_map[leaning] for leaning in desired_order]
bars = plt.bar(desired_order, high_active_counts, color=colors, edgecolor='black')
...
medium_active_counts = medium_active_counts.reindex(desired_order, fill_value=0)
colors = [color_map[leaning] for leaning in desired_order]
bars = plt.bar(desired_order, medium_active_counts, color=colors, edgecolor='black')
...
low_active_counts = low_active_counts.reindex(desired_order, fill_value=0)
colors = [color_map[leaning] for leaning in desired_order]
bars = plt.bar(desired_order, low_active_counts, color=colors, edgecolor='black')
